...
### 5.1 临床预警决策模型全图 (基于非线性风险分层树与矩阵)
![5_1预警决策流程图](MathorCup/5_1_高质预警决策流程图_Tex专用.png)

*注：此组流程图提供 3 种独立格式（尺寸更贴合 LaTeX 单双栏），供正文直接填入或进一步的二次编辑：*
* **1. 高清静态图（适合直接填入 Tex 中的 png）：** `MathorCup/5_1_高质预警决策流程图_Tex专用.png`
* **2. 纵向排版交互版（柔和色彩圆角切边 drawio）：** `MathorCup/5_1_高质预警决策流程图_Tex纵向排版.drawio`
* **3. 纵向排版交互版（方正硬派直角强原色 drawio）：** `MathorCup/5_1_预警决策流程图_原版色直角纵向排版.drawio`

> **图注描述**：本图依据医学界针对十年冠心病等心血管代谢疾病（ASCVD）典型的风险评估阶梯逻辑，深度结合了本课题所特有的“核心血脂异常、异常痰湿积水平、日常活动能力受限(ADL)总量”这一极具多维特征的三级框架，从代码底层直接构建了高阶的非线性阈值预警网络。
> 图表的上半程（初筛期）为合并重大危急病史/完全活动受限样本设置了极高危专道，实行一键强干预直通；下半程则充分利用了随机森林基尼（Gini）重要度和互信息（MI）度量结果，通过绿（安全低危区）、黄（预警中度区）、红（超标高危区）色块矩阵清晰剖析了“特征交叉关联影响”。直观展现出即使在“血脂生化指数表现正常”时，一旦叠加“超高浓度的痰湿体质表现与显著的活动受限”，依然会发生向红区的穿透性致病力。这就为问题三制定的优化调理干预门槛赋予了强大的解释学依据。
...
## 4. 阶段工作历史总结：可视化制图与文献编排
在本次攻坚历程中，我们系统性地完成了多项学术级可视化图表的生成与定制（包括堆叠图、棒棒糖图、Upset图、以及适配 LaTeX 的多版纵向/横向预警决策流程图），同时将高质量的基础机器学习/统计学文献（互信息、随机森林等）严谨地编排进了正文中。

所有的制图代码、可编辑 `.drawio` 源文件，以及针对论文修改的汇总说明，都已汇编至下方的阶段性总结档案中，以供随时追溯、复查和提交：
👉 **[阶段工作总结：数据可视化制图与论文文献编排](MathorCup/阶段工作总结_可视化与文献编排.md)**
