...
这里使用 `PlanResult` 数据类来统一保存问题 3 的优化结果。""",
    "find_default_xlsx": """## 4. 自动寻找样例数据

这个函数会在当前项目目录里寻找 C 题的 `.xlsx` 样例文件。""",
    "load_dataframe": """## 5. 读取 Excel 为 DataFrame
...
从这里开始，后面所有建模操作都建立在 `DataFrame` 之上。""",
    "summarize_problem_1": """## 6. 问题 1：关键指标筛选与体质贡献分析
...
- 结果也更多地组织成 `DataFrame`；
- 排序和汇总会更适合后续转表格。""",
    "build_feature_matrix": """## 7. 诊断型模型特征矩阵

这个函数返回用于诊断型模型的特征矩阵。

这一步非常直观：直接用列名列表从 `df` 中取列。""",
    "build_early_warning_matrix": """## 8. 早预警模型特征矩阵

这一段依然体现“双层模型”思想：

- 诊断型模型保留直接血脂指标；
- 早预警模型去掉直接血脂指标。""",
    "fit_risk_models": """## 9. 问题 2：训练双层风险模型
...
- 特征更容易按名称解释；
- 重要特征结果也更容易整理成表。""",
    "lipid_abnormal_count": """## 10. 统计血脂异常项数

这个函数会根据题目给定的临床阈值，计算单个患者有多少项血脂异常。""",
    "build_early_warning_row": """## 11. 构造单个样本的早预警输入

这里把一个 `Series` 重新包装成单行 `DataFrame`，这样就能直接喂给 sklearn 模型。""",
    "assign_risk_level": """## 12. 低中高风险分层
...
- 再看痰湿积分和活动能力；
- 最后结合模型概率。""",
    "summarize_risk_levels": """## 13. 汇总风险等级分布

这个函数会遍历整张表，为每个样本分层，然后统计低、中、高风险的样本量。""",
    "tcm_level": """## 14. 中医调理等级映射

根据痰湿积分，把患者映射到 1 / 2 / 3 级调理方案。""",
    "allowed_intensities": """## 15. 可选活动强度约束

这一段把题目里的年龄约束和活动能力约束写成代码。""",
    "activity_monthly_drop": """## 16. 月度降分比例

这一段把题目里的“强度提升”和“频次增加”对痰湿积分的影响翻译成公式。""",
    "simulate_plan": """## 17. 模拟某个方案的 6 个月过程

给定强度和频次，这个函数会逐月模拟痰湿积分变化和成本变化。""",
    "optimize_intervention": """## 18. 问题 3：搜索最优干预方案

这一步通过枚举全部可行方案，最后选出满足目标且成本最优的那一个。""",
    "print_problem_1": """## 19. 问题 1 输出函数

把问题 1 的核心结果打印得更清晰。""",
    "print_problem_2": """## 20. 问题 2 输出函数

把问题 2 的模型表现、重要特征和分层结果集中打印出来。""",
    "print_problem_3": """## 21. 问题 3 输出函数

把指定样本的最优方案和逐月明细打印出来。""",
    "main": """## 22. 命令行入口

如果你在终端里直接运行 `c_problem_starter.py`，就是从这里开始执行。""",
    "__main_block__": """## 23. 入口判断
